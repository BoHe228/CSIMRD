...
def MobileNetV2Block(in_channels, out_channels, n=1, stride=1, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, activation='relu6'):
        convs = []
...
class CSCSFFB(nn.Module):
    def __init__(self, in_dims, target_dim, target_size):
        ...
        self.channel = VRWKV_ChannelMix(n_embd=target_dim*2, channel_gamma=1/4, shift_pixel=1, hidden_rate=2)
    ...
    def forward(self, features):
        ...
        # print(f"Patch resolution: {patch_resolution}")
        # print(f"original_sizes resolution: {self.original_sizes}")
...
class MobCSC_Decoder(nn.Module):
    def __init__(self, channels=[512,320,128,64], kernel_sizes=[1,3,5], dw_parallel=True, activation='relu6',expansion_factor=2,
                 chunk = 1,ks = 3):
        super(MobCSC_Decoder,self).__init__()
        self.mscb4 = MobileNetV2Block(channels[0], channels[0], n=1, stride=1, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel, activation=activation)

        self.eucb3 = UDCB(in_channels=channels[0], out_channels=channels[1], kernel_size=ks, stride=ks//2)
        self.mscb3 = MobileNetV2Block(channels[1], channels[1], n=1, stride=1, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel,  activation=activation)

        self.eucb2 = UDCB(in_channels=channels[1], out_channels=channels[2], kernel_size=ks, stride=ks//2)
        self.mscb2 = MobileNetV2Block(channels[2], channels[2], n=1, stride=1, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel,  activation=activation)
        
        self.eucb1 = UDCB(in_channels=channels[2], out_channels=channels[3], kernel_size=ks, stride=ks//2)
        self.mscb1 = MobileNetV2Block(channels[3], channels[3], n=1, stride=1, kernel_sizes=kernel_sizes, expansion_factor=expansion_factor, dw_parallel=dw_parallel, activation=activation)
