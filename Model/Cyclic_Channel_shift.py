...
class OHDC(nn.Module):
    def __init__(self, in_channels, out_channels,kernel_sizes, stride, activation='relu6', dw_parallel=True):
        ...
        self.dwcs = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(self.in_channels, self.in_channels, kernel_size, self.stride,
    ...
    def forward(self, x):
        ...
        for dwc in self.dwcs:
            dwc_out = dwc(x)
            out = out + dwc_out  
            if self.dw_parallel == False:
                x = x+dw_out
        output = channel_shuffle_OHDC(out, GConv_groups(self.in_channels,self.out_channels))
        return output
...
class ShuffleV2Block(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, kernel_sizes=[1,3,5],expansion_factor=2, 
        ...
        assert chunk in [0, 1], "chunk must be 1 or 0"
        ...
        if self.chunk == 1:
            self.ex_channels = int(self.mid_channels * self.expansion_factor)
            self.in_branch = self.mid_channels
            self.out_branch = self.mid_channels
            if stride == 2 :
        ...
        if self.chunk == 0:
            self.in_branch = self.in_channels
            self.ex_channels = int(self.in_channels * self.expansion_factor)
            self.out_branch = self.out_channels
                            
        self.branch2 = nn.Sequential(
                nn.Conv2d(self.in_branch, self.ex_channels,
                          kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.ex_channels),
                act_layer(self.activation, inplace=True),
                OHDC(self.ex_channels,self.out_branch ,kernel_sizes=self.kernel_sizes, stride=self.stride,
                     activation=self.activation, dw_parallel=self.dw_parallel),  
                nn.Conv2d(self.ex_channels, self.out_branch, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(self.out_branch)
            )
    ...
    def forward(self, x):
        ...
        if self.chunk == 1:
            if self.stride == 1:
                ...
                # import ipdb; ipdb.set_trace()
                out = torch.cat((x1, self.act_layer(self.branch2(x2))), dim=1)
            else:
                out = torch.cat((self.branch1(x), self.branch2(x)), dim=1)
            output = self.channel_shuffle(out)
        else:
            # import ipdb; ipdb.set_trace()
            out = self.branch2(x)
            output = x + out
...
def Cyclic_Channel_shift(input, shift_pixel=1, gamma=1/4, patch_resolution=None):
    assert gamma <= 1/4
    B, N, C = input.shape
...
class ChannelMix(nn.Module):
    ...
    def forward(self, x, patch_resolution=None):
        if self.shift_pixel > 0:
            ...
            xx = Cyclic_Channel_shift(x, self.shift_pixel, self.channel_gamma, patch_resolution)
