...
from Model.MRCFD_Decoder import MRCFD_Decoder
from Model.MobCSC_Decoder import MobCSC_Decoder
# from Baseline_CSFM_ShuffleNet import MRCFD_Decoder
# from NO_Channel_Shift import MobCSC_Decoder


#MECFD:Efficient Multi-Scale Feature Extraction and Cross-Scale Fusion Decoding for Medical Image Segmentation
class MRCFDNet(nn.Module):
    def __init__(self, num_classes=1, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, activation='relu6', encoder='pvt_v2_b2', pretrain=True,chunk = 1,ks = 3):
        super(MRCFDNet, self).__init__()
        ...
        if encoder == 'pvt_v2_b0':
            ...
            path = 'Model/pretrained_pth/pvt/pvt_v2_b2.pth'
            channels=[512, 320, 128, 64]
        ...
        self.decoder = MobCSC_Decoder(channels=channels, kernel_sizes=kernel_sizes, dw_parallel=dw_parallel,
                                     expansion_factor=expansion_factor,activation=activation,chunk = chunk,ks = ks)
