        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                if test_save_path is not None and case is not None:
                    ...
                    plt.imshow(pred, cmap=cmap, vmin=0, vmax=classes - 1)
                    plt.axis('off')
                    save_name = os.path.join(test_save_path, f'{case}_{ind}_pred_mask_only.png')
                ...
                # Close the figures to release memory
                # plt.close(fig_gt)
                # plt.close(fig_pred)
